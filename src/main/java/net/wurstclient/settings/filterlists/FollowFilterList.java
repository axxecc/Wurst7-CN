...
public final class FollowFilterList extends EntityFilterList
{
	...
	public static FollowFilterList create()
	{
		...
		builder.add(new FilterSleepingSetting("不会跟随正在睡觉的玩家", false));
		
		builder.add(new FilterFlyingSetting("不会跟随距离地面至少一定距离的玩家", 0));
		
		builder.add(new FilterHostileSetting("不会跟随僵尸和苦力怕等敌对怪物", true));
		
		builder.add(FilterNeutralSetting.onOffOnly("不会跟随末影人和狼等中立生物", true));
		
		builder.add(new FilterPassiveSetting(
			"不会跟随猪和牛这样的动物, 不会追踪蝙蝠这样的环境生物, 也不会追踪像鱼、鱿鱼和海豚这样的水生物", true));
		
		builder.add(
			new FilterPassiveWaterSetting("不会跟随被动的水生物, 比如鱼, 鱿鱼, 海豚和美西螈", true));
		
		builder.add(new FilterBabiesSetting("不会跟随小猪, 小村民等", true));
		
		builder.add(new FilterBatsSetting("不会跟随蝙蝠和任何其他可能由模组添加的\"环境\"怪物", true));
		...
		builder.add(new FilterAllaysSetting("不会跟随悦灵", true));
		
		builder.add(new FilterInvisibleSetting("不会跟随看不见的实体", false));
