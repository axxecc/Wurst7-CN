	...
	IsPlayerInWaterListener, AirStrafingSpeedListener, MouseScrollListener
{
	private final SliderSetting horizontalSpeed = new SliderSetting("水平速度",
		"使用飞行时的水平移动速度", 1, 0.05, 10, 0.05, ValueDisplay.DECIMAL);
	
	private final SliderSetting verticalSpeed =
		new SliderSetting("垂直速度", "使用飞行时的垂直移动速度(相对于水平移动)", 1, 0.05, 5, 0.05,
	...
	private final CheckboxSetting allowUnsafeVerticalSpeed =
		new CheckboxSetting("不安全的垂直速度",
			"在生存模式下, 垂直速度可以超过3.95, 即使开启NoFall也会受到坠落伤害", false);
	...
	private final CheckboxSetting renderSpeed =
		new CheckboxSetting("列表中显示速度", "在HackList中显示你当前的水平和垂直速度", true);
	
	private final CheckboxSetting antiKick =
		new CheckboxSetting("防踢", "它会让你偶尔下降一下, 防止被踢", false);
	
	private final SliderSetting antiKickInterval =
		new SliderSetting("防踢间隔", "反踢应该多久防止你被踢一次?\n大多数服务器会在80Tick后踢你", 70, 5,
			80, 1, ValueDisplay.INTEGER.withSuffix(" Tick"));
	
	private final SliderSetting antiKickDistance =
		new SliderSetting("防踢距离", "防踢应该让你摔多远\n大多数服务器至少需要0.032m才能阻止你被踢", 0.035,
