	...
	implements UpdateListener, RenderListener
{
	...
	private final SliderSetting maxAttempts = new SliderSetting("最大尝试次数",
		"随机建造在一次更新中尝试放置块的最大随机位置数\n\n较高的值会加快构建过程, 但代价是增加延迟", 128, 1, 1024, 1,
		ValueDisplay.INTEGER);
	
	private final CheckboxSetting checkItem = new CheckboxSetting("检查保留物品",
		"仅在您实际持有方块时构建\n关闭此选项以使用火、水、熔岩、刷怪蛋构建, 或者如果您只想在随机位置用空手右键单击", true);
	
	private final CheckboxSetting checkLOS =
		new CheckboxSetting("检查视线", "确保随机建造不会尝试在墙后放置块", false);
